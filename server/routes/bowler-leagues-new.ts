...
router.get("/", async (req, res) => {
  try {
    ...
    console.log('[BowlerLeaguesNew] Fetching with params:', { bowlerId, leagueId, teamId });
    ...
    // Fetch additional data for each bowler league to return more comprehensive information
    const enrichedBowlerLeagues = await Promise.all(
      bowlerLeagues.map(async (bl) => {
        const [bowler, team, league] = await Promise.all([
          storage.getBowler(bl.bowlerId),
          storage.getTeam(bl.teamId),
          storage.getLeague(bl.leagueId),
        ]);
    ...
    console.log(`[BowlerLeaguesNew] Found ${enrichedBowlerLeagues.length} enriched bowler leagues for filters:`, filters);
    sendSuccess(res, enrichedBowlerLeagues);
  } catch (error) {
    console.error('[BowlerLeaguesNew] Error:', error);
...
router.post("/", async (req, res) => {
  try {
    console.log('[BowlerLeaguesNew] Creating new bowler league with body:', req.body);
    ...
    console.log('[BowlerLeaguesNew] Created bowler league:', created);
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    console.log(`[BowlerLeaguesNew] Updating bowler league ${id}:`, update);
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    console.log(`[BowlerLeaguesNew] Deleting bowler league ${id}`);
    ...
    console.log(`[BowlerLeaguesNew] Successfully deleted bowler league ${id}`);
    sendSuccess(res, { message: "Bowler league deleted successfully" }, 200);
  } catch (error) {
    console.error('[BowlerLeaguesNew] Error deleting bowler league:', error);
