...
- **Schema-level guard (leagues)**: `leagues.organization_id` is `NOT NULL` — the database refuses to create an org-less league. The insert/update zod schemas in `shared/schema/leagues.ts` mirror this (org id is required and non-nullable). The legacy `globalAccess: true` branch in `POST /api/leagues` (which created `organization_id = NULL` rows for system admins) has been removed; system admins must specify the target organization explicitly.
- **Schema-level guard (users)**: `users.organization_id` stays nullable (the bootstrap system_admin legitimately has no org) but a `users_role_org_required` CHECK constraint enforces `role = 'system_admin' OR organization_id IS NOT NULL` (migration `0003_users_org_required_for_non_admin.sql`, mirrored in `shared/schema/users.ts`). The schema's `insertUserSchema` rejects non-admin inserts without an `organizationId`, and `server/storage/users.ts` exports a typed `NonAdminMissingOrgError` that `createUser`, `updateUserRole`, and `setUserOrganization` throw before hitting the database. Specifically:
  ...
  - `DELETE /api/admin/users/:id/remove` (organization admin "remove from org") returns `400 ORG_REQUIRED`; reassign or delete the account instead of orphaning it.
