...
- **Per-Fork Test DB Clone Memo via `process.env`**: For vitest projects that combine `pool: 'forks'` with `isolate: true` (`parallel-isolated`, `serial-fk-bypass`), the per-worker template-DB clone is memoized in reserved `process.env` keys (`__LV_WORKER_DB_NAME__`, `__LV_WORKER_DB_URL__`, `__LV_WORKER_APP_PORT__`) rather than module state, because `isolate: true` resets module-level memos between files within the same fork while `process.env` survives. This keeps the clone count at ≤ `maxForks` instead of ~one-per-file.
