...
| `POST /api/leagues` | body `organizationId` (system_admin) / body `locationId` | `leagues.organization_id` / `leagues.location_id` | Existence + same-tenant for `locationId` added in #454 (404 NOT_FOUND). Org existence verified for both session-derived and override branches |
| `PATCH /api/leagues/:id` | body `organizationId` (system_admin) / body `locationId` | `leagues.organization_id` / `leagues.location_id` | Existence checks added in #454. Non-system-admin org change still 403s |
| `POST /api/locations` | body `organizationId` (system_admin) | `locations.organization_id` | Existence check added in #454. Non-system-admin pinned to session org |
| `POST /api/org-admin/users/:id/add` | body `organizationId` (system_admin) | `users.organization_id` | Existence check added in #454. Org-admin branch reuses session org |
| `POST /api/org-admin/users/create` | body `organizationId` (system_admin) / body `locationId` | `users.organization_id` / `users.location_id` | Existence + same-tenant checks added in #454 |
| `PATCH /api/org-admin/users/:id/location` | body `locationId` | `users.location_id` | Existence + same-tenant check added in #454. Pinned by `tests/api/admin-fk-id-existence.test.ts` |
